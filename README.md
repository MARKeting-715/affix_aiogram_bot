...
## Cambridge Dictionary (необязательно)

У Cambridge Dictionary нет API для списка аффиксов или выгрузки всех слов: API умеет искать отдельные слова. Если указать `CAMBRIDGE_ACCESS_KEY`, бот при старте проверит до 500 подходящих слов через официальный API и добавит найденные русские переводы к вопросам. Ключ нужно запросить у Cambridge и хранить только в `.env`.
